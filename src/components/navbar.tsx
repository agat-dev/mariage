...
export default function Navbar({ className = '' }: NavbarProps) {
  ...
  const navItems = [
    ...
    { name: 'Infos Pratiques', href: '#info' },
    { name: 'RSVP', href: '#rsvp' }
  ];
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item, index) => (
                <motion.a
                  key={item.name}
                  href={item.href}
                  className="text-white/80 hover:text-white px-3 py-2 text-sm font-poiret-one tracking-wide transition-colors duration-200 relative group"
                  whileHover={{ scale: 1.05 }}
                ...
                >
                  ...
                  {/* Underline effect */}
                  <span className="absolute inset-x-0 bottom-0 h-0.5 bg-white transform scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left" />
